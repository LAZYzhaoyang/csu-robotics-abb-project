...
while isstart == True:
    ...
    cnts1,_=cv2.findContours(img_cp,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)#轮廓检测，输入二值图，cnts为返回的轮廓
    anum=len(cnts1)
    cutnum=0
    center1=np.zeros([anum,2],dtype=np.int32)
    for i in range(0,anum):
        cnt_second=sorted(cnts1,key=cv2.contourArea,reverse=True)[i]#按照轮廓的面积从大到小进行排序，输出第a个轮廓
        ...
        center1[i][0]=int(center_x)
        center1[i][1]=int(center_y)
        cutsize=75
        ...
        for i in range(0,anum):
            if center1[i][0]!=0:
                centernew[newnum][0]=center1[i][0]
                centernew[newnum][1]=center1[i][1]
                newnum=newnum+1
        anum=anum-cutnum
        center1=centernew
    passnum=0
    for i in range(0,anum):
        ...
        if thingy1-worldy0<=0:
            ...
            dyw1=-dyw1
        dxw1,dyw1=dyw1,dxw1
        if dxw1>=300 or dyw1>=300:
            print >> 'too far cannot get it'
            continue
        #camera2摄影高度
        hight1=300
        sendwords1=str(dxw1)+','+str(dyw1)+',0,'+str(hight1)+',0,1/0D'
        ...
        while ifopencamera2==1:
            ...
            if (data == 'pass'):
                ...
                break
             # Get the image. It is a numpy array  获取图像。它是一个numpy数组
            image2 = Tis2.Get_image()
            # Display the result
            cv2.imshow('Window2', image2)
            cv2.waitKey(5000)
            # 去畸变
            h, w = image2.shape[:2]
            newcameramtx2, roi2 = cv2.getOptimalNewCameraMatrix(mtx2, dist2, (w, h), 0, (w, h))  # 自由比例参数
            dst2 = cv2.undistort(image2, mtx2, dist2, None, newcameramtx2)
            # 根据前面ROI区域裁剪图片
            x, y, w, h = roi2
            dst2 = dst2[y:y + h, x:x + w]
            cv2.imwrite('camera2.jpg', dst2)
            # image reconized
            path = 'camera2.jpg'
            imgc2 = cv2.imread(path)
            # 缩小图像
            height2, width2 = imgc2.shape[:2]
            size = (int(width2 * 0.1), int(height2 * 0.1))
            imgc2 = cv2.resize(imgc2, size, interpolation=cv2.INTER_AREA)
            gray = cv2.cvtColor(imgc2, cv2.COLOR_BGR2GRAY)  # 转灰度图
            hsv = cv2.cvtColor(imgc2, cv2.COLOR_BGR2HSV)  # 转hsv色彩空间
            # get black area
            # ret,thresh=cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C ,cv2.THRESH_BINARY,11) #阈值化处理，大于148的变为白色（0为黑色，255为白），输入灰度图
            ret, thresh = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY)  # 阈值化处理，大于148的变为白色（0为黑色，255为白），输入灰度图
            mask = thresh  # 构建掩模
            ...
            # 获取中心区域轮廓及坐标
            img_cp = morph.copy()  # 复制原来的图像到一张新的图像上
            cnts2, _ = cv2.findContours(img_cp, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # 轮廓检测，输入二值图，cnts为返回的轮廓
            camera2num = len(cnts2)
            center2 = np.zeros([camera2num, 2], dtype=np.int32)
            for i in range(0, camera2num):
                cnt_second = sorted(cnts2, key=cv2.contourArea, reverse=True)[i]  # 按照轮廓的面积从大到小进行排序，输出第a个轮廓
                box = cv2.minAreaRect(cnt_second)  # 最小矩形面积
                points = np.int0(cv2.boxPoints(box))  # 查找旋转矩形的 4 个顶点，points为四个顶点坐标
                mask = np.zeros(gray.shape, np.uint8)  # 生成灰度图同样大小的零矩阵，uint8是无符号八位整型,表示范围是[0, 255]的整数
                # 进行轮廓的颜色填充，第一个参数是一张图片，可以是原图或者其他。第二个参数是轮廓，一个列表。第三个参数是对轮廓（第二个参数）的索引，当需要绘制独立轮廓时很有用，若要全部绘制可设为-1。接下来的参数是轮廓的颜色和厚度。
                ...
                p3x, p3y = points[2,0], points[2,1]
                center_x, center_y = (p1x + p3x)/ 2, (p1y + p3y)/ 2  # 得到中心坐标
                center2[i][0] = int(center_x)
                center2[i][1] = int(center_y)
            u0c2=math.floor(mtx2[0][2])
            v0c2=math.floor(mtx2[1][2])
            centernum=0
            mindpoint=sys.maxsize
            for i in range(0, camera2num):
                thingx2 = center2[i][0]
                thingy2 = center2[i][1]
                dx2=thingx2-u0c2
                ...
                if d2<=mindpoint:
                    mindpoint=d2
                    centernum=i
            thingx2 = center2[centernum][0]
            thingy2 = center2[centernum][1]
            # 图像中心至物体所在平面的距离(单位：mm)
            dheight=50
            Zc2 = hight1-dheight
            # 相机内参
            ...
            fy2 = mtx2[1][1]
            # 在相机坐标系下，目标中心坐标与原点的实际坐标差值
            dxc2 = Zc2/fx2*abs(thingx2-u0c2)
            dyc2 = Zc2/fy2*abs(thingy2-v0c2)
            # 由相机坐标系转为世界坐标系
            numrvecs2 = len(rvecs2)
            transx2 = 0
            transy2 = 0
            transz2 = 0
            for i in range(0, numrvecs2):
                transx2 = transx2 + rvecs2[i][0]
                transy2 = transy2 + rvecs2[i][1]
                transz2 = transz2 + rvecs2[i][2]
            transx2 = transx2 / numrvecs2
            transy2 = transy2 / numrvecs2
            transz2 = transz2 / numrvecs2
            transx2 = math.radians(transx2)
            transy2 = math.radians(transy2)
            transz2 = math.radians(transz2)
            Rx2 = np.array(
                [[1, 0, 0], [0, math.cos(transx2), math.sin(transx2)], [0, -math.sin(transx2), math.cos(transx2)]])
            Ry2 = np.array(
                [[math.cos(transy2), 0, math.sin(transy2)], [0, 1, 0], [-math.sin(transy2), 0, math.cos(transy2)]])
            Rz2 = np.array(
                [[math.cos(transz2), math.sin(transz2), 0], [-math.sin(transz2), math.cos(transz2), 0], [0, 0, 1]])
            trans2 = np.dot(np.dot(Rx2, Ry2), Rz2)
            dpoint2 = np.array([dxc2, dyc2, 1])
            dpoint2 = dpoint2.T
            dw2 = np.dot(trans1, dpoint1)
            dxw2 = dw2[0]
            dyw2 = dw2[1]
            if thingx2-u0c2 <= 0:
                dxw2 = dxw2
            ...
            dxw2=dxw1+dxw2+dxc2
            dyw2=dyw1+dyw2+dyc2
            if dxw2>=300 or dyw2>=300:
                print >> 'too far cannot get it'
                break
            
            
            thingheight=15
            drophight2 = 0+passnum*thingheight
            passnum=passnum+1
            sendwords2 = str(dxw2) + ',' + str(dyw2) + ',0,0,1,0/0D'
            pickuppoint = str(dxw2) + ',' + str(dyw2) + ',0,' + str(hight1) + ',1,0/0D'
            putpointup='200,200,0,'+str(hight1)+',1,0/0D'
            putdownpoint='200,200,0,'+str(drophight2)+',1,0/0D'
            putdown='200,200,0,'+str(drophight2)+',2,0/0D'
            retools='0,-300,0,200,0,0/0D'

            data = connection.recv(16)
            print >> sys.stderr, 'received "%s"' % data
            if (data == 'pass'):
                print >> sys.stderr, 'sending data back to the client'
                connection.sendall(sendwords2)
                print >> 'wait next command'
    ...
    connection.close()
    isend=input('is end this program?yes:1,no:0')
    if isend == 1:
        isstart = False
